...
struct Msg
{
	...
	static Msg parse(ubyte[] srcData)
	{
		...
		auto enc = m.headers.all("content-transfer-encoding").length
			? m.headers["content-transfer-encoding"].toLower : "";

		if ((cast(string) data).strip.empty)
			return m;
		//	Transfer encoding
		switch (enc)
		{
		...
		if (!ct.length)
		{
			...
			if (ctl.startsWith("multipart/related")
				|| ctl.startsWith("multipart/alternative")
				|| ctl.startsWith("multipart/mixed"))
			{
